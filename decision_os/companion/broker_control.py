...
class ActivationTuple:
    ...
    def __post_init__(self) -> None:
        for value, label in (
            ...
            (self.repository_id, "Repository identity"),
            (
        ...
        if (
            ...
            or self.generation_witness < 0
        ):
            raise ValueError("Generation witness must be a non-negative integer.")
...
class MutationDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.operation) not in {MutationOperation, str}:
...
class TargetObservation:
    ...
    def __post_init__(self) -> None:
        if type(self.kind) not in {TargetKind, str}:
            raise MutationDecisionError(
                "Target observation kind is invalid."
            )
        try:
...
def reconcile_mutation(
    ...
    if (
        type(decision) is not MutationDecision
        or type(observation) is not TargetObservation
    ):
        raise MutationDecisionError(
            "Reconciliation requires one strict decision and observation."
        )
...
def _complete_cas_intent(
    ...
    if outcome not in {
        ...
        raise ControlRecordIntegrityError(
            "Only an exact CAS outcome can complete an authority fence."
        )
    payload: dict[str, Any] = {
        "schema": CAS_FENCE_SCHEMA,
        "kind": "COMPLETE",
...
class ControlDomainStore:
    ...
    def _load_unlocked(
        ...
        *,
        ...
        if head is None and not journal:
            ...
            return None
        if head is None or not journal or head != journal[-1]:
            raise ControlRecordIntegrityError(
    ...
    def _load_for_abandonment_unlocked(
        self,
    ) -> tuple[ControlDomainRecord, bool]:
        """Return the journal authority while permitting only safe retirement.

        A crash can make the append-only journal durable before publishing its
        replaceable head.  Such a prefix must never authorize mutation, but it
        must remain possible to abandon the newest domain.  The head may
        therefore be absent or an exact earlier journal record on this path;
        any unrelated head remains an integrity failure.
        """
        ...
        if not journal:
            raise ControlRecordIntegrityError(
                "Persisted Broker control journal is absent."
            )
        if head is not None and head not in journal:
            raise ControlRecordIntegrityError(
                "Broker control-record head is not part of its durable journal."
            )
    ...
    def transition(
        ...
        try:
            if type(target_state) not in {ControlDomainState, str}:
                raise TypeError
        ...
        with self._locked(exclusive=True):
            ...
            if target is ControlDomainState.ABANDONED:
                current, head_is_current = (
                    self._load_for_abandonment_unlocked()
                )
            else:
                current = self._load_required_unlocked()
            self._require_exact_activation(current, expected_activation)
            if (
                target is ControlDomainState.ABANDONED
                and current.state is ControlDomainState.ABANDONED
                and not head_is_current
            ):
                self._durable_publish_unlocked(
                    self.path,
                    _canonical_record_bytes(current),
                    replace_existing=True,
                )
                return self._load_required_unlocked()
    ...
    def _require_exact_activation(
        ...
        if (
            type(expected) is not ActivationTuple
            or current.activation != expected
        ):
    ...
    def authorize_cas(
        ...
        """Fence before consuming observation data; never apply repository bytes."""
        ...
        if (
            ...
            raise MutationDecisionError(
                "CAS authorization requires one bound decision and observation."
            )
        with self._locked(exclusive=True):
            current = self._load_required_unlocked()
            ...
            if current.state is not ControlDomainState.ACTIVE:
                raise AuthorityRejectedError(
                    "Current Broker control domain cannot authorize mutation."
                )
            journal = self._journal_records_unlocked()
            ...
            if outcome is ReconciliationOutcome.UNCERTAIN:
                uncertain = self._prospective_record(
                    current,
                    ControlDomainState.UNCERTAIN,
                )
                self._publish_unlocked(uncertain)
                return outcome
            completion = _complete_cas_intent(intent, outcome)
            self._append_cas_fence_unlocked(completion)
            if self._load_required_unlocked(
                allow_consumed_cas=True,
            ) != current:
                raise ControlRecordIntegrityError(
                    "Broker CAS completion changed its control authority."
                )
            return outcome
